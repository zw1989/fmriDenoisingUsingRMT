...
for jj=which_dims
    if size(z,jj) < length(kernel) % The size along one dimension needs to be larger than the kernel size
        %continue
        kerneltmp = downsample(kernel,ceil(length(kernel)/size(z,jj))); % Whether downsample?
    else
        kerneltmp = kernel;
    end
    z=convn(z,permute(kerneltmp,circshift((1:max(2,jj)),[0 jj-1])),'valid');
    % figure; histogram(z(:));
end
